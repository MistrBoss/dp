...
Do původního řešení byla zavedena kontrola existence totožné fotografie na základě MD5. Hodnota MD5 je ukládána do DB. Dále se nově při importu fotografie ukládají její vybraná metadata. Ty později s výhodou používají na výpočet KoZ přímo v SQL dotazu. Jedná se o:
\begin{itemize}
	\setlength{\parskip}{0pt}
	\setlength{\itemsep}{0pt}
	\setstretch{1.05}
...
Nyní budou představeny postupně jednotlivé kroky.
...
\n{4}{Matice ostrosti}
je dána maticí hran, která se získá převodem světlostní matice do~vlnového spektra pomocí DFT a~následnou konvolucí ve~vlnovém spektru (Kod.~\ref{FTHighKernelMatrix.java}). Grafická prezentace této operace je na~obrázku níže (Obr.~\ref{fig:sharpness-matrix}). K~docílení požadovaného stavu je nutné zvolit vhodný filtr viz Tab.~\ref{tab:convolution-high} (konvoluční jádro). Aplikované jádro bylo sestaveno experimentálně a~jeho funkčnost ověřena testováním na~lidech.
...
je dána maticí hran za prahovou hodnotou (přeostřených hran). Prahová hodnota byla opět definována (a testována) experimentálně v mnoha iteracích.
