...
Konstruktivní změny fotografie jsou takové změny, které jsou viditelné pro lidské oko, avšak nemění zásadně charakter obrázku pro strojové zpracování.

\n{3}{Návrh řešení}
Křížová korelace zrychlená pomocí diskrétní Fourierovy transformace \cite{FFT}.
...
$ \rho_{A,B} = \dfrac{(A - \mu_{A}) × (B - \mu_{B})}{\sigma_{A}\sigma_{B}} $
...
Zhodnocení vychází z benchmarkingu, který byl vyhodnocen jako pomocná analýza. Jako nejvýhodnější varianta pro realizaci vychází PC farma. Jako záložní řešení, lze využít produkční server s výpočtem na CPU. Pokud bude zvolen vhodný programovací jazyk (např. požadovaná JAVA), bude výsledné řešení přenositelné.
